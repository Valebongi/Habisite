...
// ─── Header compartido (dark theme) ──────────────────────────────────────────
const JuradoHeader: React.FC<{ titulo?: string }> = ({ titulo }) => (
  <IonHeader>
    <IonToolbar style={{ '--background': DARK, '--color': '#fff' }}>
      <IonTitle style={{ fontWeight: 700, letterSpacing: '0.02em' }}>
        {titulo ?? `Hola, ${juradoNombre()}`}
      </IonTitle>
      <IonButtons slot="end">
        <IonButton fill="clear" style={{ '--color': ORANGE, fontWeight: 600, fontSize: '0.85rem' }} onClick={handleLogout}>
          Salir
        </IonButton>
      </IonButtons>
    </IonToolbar>
    ...
    <>
      <IonHeader>
        <IonToolbar style={{ '--background': DARK, '--color': '#fff' }}>
          ...
          <IonButtons slot="end">
            <IonButton fill="clear" style={{ '--color': ORANGE }} onClick={onClose}>Cerrar</IonButton>
          </IonButtons>
        </IonToolbar>
      </IonHeader>
    ...
    <IonPage>
      ...
      <IonContent style={{ '--background': '#f4f5f7' }}>
        ...
        <div style={{ display: 'flex', gap: 8, padding: '4px 12px 8px', alignItems: 'center' }}>
          <IonChip
            color={soloSinEvaluar ? 'warning' : 'medium'}
                  ...
                  <IonCard key={p.id} style={{ margin: '8px 0', opacity: evaluado ? 0.85 : 1 }}
                    ...
                    <IonItem lines="none">
                      <div slot="start" style={{
                        ...
                        color: evaluado ? '#2dd36f' : ORANGE, flexShrink: 0,
                      }}>
                        #{numGlobal}
                      </div>
                      <IonLabel>
          ...
          <>
            {evaluaciones.length > 0 && (
              <div style={{ display: 'flex', gap: 8, padding: '12px 12px 4px', flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 80, background: '#fff', borderRadius: 12, padding: '14px 14px', textAlign: 'center', border: '1px solid #e5e7eb' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: ORANGE }}>{evaluaciones.length}</div>
                  <div style={{ fontSize: '0.72rem', color: '#9ca3af' }}>Evaluaciones</div>
                </div>
                <div style={{ flex: 1, minWidth: 80, background: '#fff', borderRadius: 12, padding: '14px 14px', textAlign: 'center', border: '1px solid #e5e7eb' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#2dd36f' }}>{promedio}</div>
                  <div style={{ fontSize: '0.72rem', color: '#9ca3af' }}>Promedio</div>
                </div>
                <div style={{ flex: 1, minWidth: 80, background: '#fff', borderRadius: 12, padding: '14px 14px', textAlign: 'center', border: '1px solid #e5e7eb' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#16a34a' }}>{maximo}</div>
                  <div style={{ fontSize: '0.72rem', color: '#9ca3af' }}>Maximo</div>
                </div>
                <div style={{ flex: 1, minWidth: 80, background: '#fff', borderRadius: 12, padding: '14px 14px', textAlign: 'center', border: '1px solid #e5e7eb' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#6b7280' }}>{minimo}</div>
                  <div style={{ fontSize: '0.72rem', color: '#9ca3af' }}>Minimo</div>
                </div>
              </div>
            )}
                    ...
                    <IonCard key={ev.id} style={{ margin: '8px 0' }}>
                      <IonItem lines="none">
                        ...
                        <div slot="end" style={{ textAlign: 'center', minWidth: 52 }}>
                          <div style={{ fontSize: '1.5rem', fontWeight: 700, lineHeight: 1, color: scoreColor(ev.puntaje) }}>
                            {ev.puntaje}
                          </div>
                          <div style={{ fontSize: '0.7rem', color: '#9ca3af' }}>/10</div>
                        </div>
...
const EstadisticasTab: React.FC = () => {
  ...
  // Ranking general (promedio de todos los jurados por postulante)
  const ranking = postulantes.map(p => {
    const evalsDeP = evaluaciones.filter(e => e.postulanteId === p.id);
    const avg = evalsDeP.length > 0
      ? evalsDeP.reduce((s, e) => s + e.puntaje, 0) / evalsDeP.length
      : 0;
    return { postulante: p, promedio: avg, totalEvals: evalsDeP.length };
  })
    .filter(r => r.totalEvals > 0)
          ...
          <div style={{ padding: '12px 12px 80px' }}>
            ...
            <div style={{ background: '#fff', borderRadius: 14, padding: '18px 20px', marginBottom: 14, border: '1px solid #e5e7eb' }}>
              ...
              <div style={{ display: 'flex', gap: 12, marginBottom: 14 }}>
                <div style={{ flex: 1, textAlign: 'center', background: '#f9fafb', borderRadius: 10, padding: '12px 8px' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: '#16a34a' }}>{evaluados}</div>
                  <div style={{ fontSize: '0.7rem', color: '#6b7280' }}>Evaluados</div>
                </div>
                <div style={{ flex: 1, textAlign: 'center', background: '#f9fafb', borderRadius: 10, padding: '12px 8px' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: ORANGE }}>{pendientesMios}</div>
                  <div style={{ fontSize: '0.7rem', color: '#6b7280' }}>Pendientes</div>
                </div>
                <div style={{ flex: 1, textAlign: 'center', background: '#f9fafb', borderRadius: 10, padding: '12px 8px' }}>
                  <div style={{ fontSize: '1.6rem', fontWeight: 700, color: DARK }}>{totalPostulantes}</div>
              ...
              <p style={{ margin: '8px 0 0', fontSize: '0.75rem', color: '#9ca3af', textAlign: 'center' }}>
                {evaluados === totalPostulantes ? 'Completaste todas las evaluaciones' : `${Math.round((evaluados / totalPostulantes) * 100)}% completado`}
              </p>
            </div>
                    ...
                    <div key={d.score} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                      ...
                      <div style={{
                        ...
                        background: d.count > 0
                          ? d.score >= 7 ? '#2dd36f' : d.score >= 5 ? ORANGE : '#eb445a'
                          : '#f3f4f6',
            ...
            {ranking.length > 0 && (
              <div style={{ background: '#fff', borderRadius: 14, padding: '18px 20px', border: '1px solid #e5e7eb' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
                  ...
                  <span style={{ fontSize: '0.72rem', color: '#9ca3af', marginLeft: 'auto' }}>Promedio de todos los jurados</span>
                </div>
                {ranking.slice(0, 10).map((r, i) => (
                  <div key={r.postulante.id} style={{
                    ...
                    <div style={{
                      ...
                      color: i < 3 ? '#fff' : '#6b7280', flexShrink: 0,
                    }}>
                      {i + 1}
                    </div>
                    ...
                    <div style={{ textAlign: 'center', flexShrink: 0 }}>
                      <div style={{ fontSize: '1.3rem', fontWeight: 700, color: r.promedio >= 7 ? '#16a34a' : r.promedio >= 5 ? ORANGE : '#eb445a', lineHeight: 1 }}>
